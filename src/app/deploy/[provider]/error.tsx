...
export default function DeployDetailError({
  error,
  reset,
}: {
  ...
  return (
    <div className="flex min-h-[40vh] flex-col items-center justify-center text-center">
      <h2 className="font-display mb-3 text-2xl font-bold text-sec-red">
        Failed to load deploy option
      </h2>
      <p className="mb-6 text-sm text-text-secondary">
        {error.digest
          ? `Something went wrong (${error.digest})`
          : "The deploy option could not be loaded. It may not exist or there was a server error."}
      </p>
